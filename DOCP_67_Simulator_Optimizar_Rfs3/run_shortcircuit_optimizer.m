function [index] =  run_shortcircuit_optimizer(L)
global Rf bdat ldat tdat  K Ip Ibase econv itermax nr  
...
%vt=ones(1,length(ldat2(:,1))+ngen+0*length(tdat(:,1)));% prfault voltages set equal to 1
...
%  Zbr=inv(Ybxx);
%  size(Yb0);
% %  U(:,10)=-Zbr(:,10).*inv(Zbr(10,10))
% % abs(Ibase*(U(5,10)-U(10,10))/Z2xx(10,5)) 
% % abs(Ibase*(U(13,10)-U(5,10))/Z2xx(13,5)) 
% % abs(Ibase*(U(11,10)-U(5,10))/Z2xx(11,5)) 
%  abs(Ibase*(U(15,10)-U(5,10))/Z2xx(15,5)) 
%  abs(Ibase*(U(1,10)-U(14,10))/Z2xx(1,14))
% % Yb0- Ybxx
% % Z2x(5,10)
% % Z2x(14,1)
% Z2x(1,7)
...
% Z2x(4,10)
% Z2x(10,5)
%    angle(Ibase*(U(5,10)-U(10,10))/Z2(5,10))*180/pi  
%   Yb(5,5)-(inv(Z2(5,10))+inv(Z2(5,11))+inv(Z2(5,13))+inv(Z2(5,15))) 
...
IndexB(k,13)=K(1,IndexB(k,3))/((IndexB(k,9)/Ip(IndexB(k,3)))^K(2,IndexB(k,3))+K(3,IndexB(k,3)));%Normal main
IndexB(k,14)=K(1,IndexB(k,1))/((IndexB(k,10)/Ip(IndexB(k,1)))^K(2,IndexB(k,1))+K(3,IndexB(k,1)));%Normal backup
IndexB(k,15)=K(1,IndexB(k,3))/((IndexB(k,11)/Ip(IndexB(k,3)))^K(2,IndexB(k,3))+K(3,IndexB(k,3)));%Trans main
IndexB(k,16)=K(1,IndexB(k,1))/((IndexB(k,12)/Ip(IndexB(k,1)))^K(2,IndexB(k,1))+K(3,IndexB(k,1)));%Trans
end
index=IndexB;  
...
%   index(kw,22)= index(kw+length(index(:,1))/2,3);
%   index(kw+length(index(:,1))/2,22)= index(kw,3) ;
% end
for kw=1:length(index(:,1))/2
